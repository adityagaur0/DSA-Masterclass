// Painters Partition 
// Given K painters to paint N boards where each painter takes one unit of time to paint one unit of a board. Design an algorithm to compute the minimum amount of time required by the K painters to paint all the N boards _ such that a painter can only paint contiguous segments of boards and a board can only be painted, at maximum,  by a single painter. 
// Constraints   
// 1 <= K <= 10   
// 1 <= N <= 10  
// 1<= length of each board <= 108 
// Example 
// Input : K=3, N=5 [1, 3, 5, 2, 4] 
// Output : 6 
